...
WSRESTFUL dadosfor DESCRIPTION "Servico REST para consultas e inclusao de dados de fornecedor no ambiente TOTVS PROTHEUS" Format APPLICATION_JSON
	...
	WSDATA datainicio       AS STRING  OPTIONAL
	WSDATA datafim          AS STRING  OPTIONAL
	WSDATA page             AS INTEGER OPTIONAL
	WSDATA pagesize         AS INTEGER OPTIONAL
	WSDATA descfor          AS STRING  OPTIONAL
	WSDATA cnpjfor          AS STRING  OPTIONAL	
...
WSMETHOD GET changedemployef WSRECEIVE datainicio, datafim, page, pagesize ,descfor,cnpjfor WSSERVICE dadosfor
	...
	::SetContentType("application/json")
		...
		IF Select("FORNECEDOR") <> 0
			FORNECEDOR->(DbCloseArea())
		ENDIF

		cQuery := "	 SELECT " 
		cQuery += "		SA2.A2_EST    AS ESTADO, "
		cQuery += "		SA2.A2_NOME   AS NOME, "
		cQuery += "		SA2.A2_CGC    AS CNPJ, "
		cQuery += "		SA2.A2_EMAIL  AS EMAIL, "
		cQuery += "		SA2.A2_DDD    AS DDD, "
		cQuery += "		SA2.A2_TEL    AS TELEFONE, "
		cQuery += "		SA2.A2_CEP    AS CEP, "
		cQuery += "		SA2.A2_END    AS ENDERECO, "
		cQuery += "		SA2.A2_BAIRRO AS BAIRRO, "
		cQuery += "		SA2.A2_MUN    AS MUNICIPIO, "
		cQuery += "		SA2.A2_NREDUZ AS NOME_FANTASIA, "
		cQuery += "		SA2.A2_TIPO   AS TIPO, "
		cQuery += "		SA2.A2_COD    AS CODIGO, "
		cQuery += "		SA2.A2_LOJA   AS LOJA, "
		cQuery += "		SA2.A2_XSITE  AS SITE, "
		cQuery += "		SA2.A2_TIPOST AS TIPOST"
		cQuery += "  FROM "+ retsqlname("SA2")+" SA2 "
		cQuery += "  WHERE SA2.D_E_L_E_T_ = ' ' "
		cQuery += "  AND SA2.A2_MSBLQL <> '1' "
	
		if valtype(cdescfor) <> "U"
			cQuery+="  AND SA2.A2_NOME LIKE '"+cdescfor+"%'"
		Endif
		If valtype(cCnpj) <> "U"
			cQuery+="  AND SA2.A2_CGC LIKE '%"+cCnpj+"%'"
		Endif
	
		cQuery := changequery(cQuery)
		tcquery cQuery new alias "FORNECEDOR"
		...
		While !FORNECEDOR->(EOF())
			...
			For nX := 1 to nTotCpo
				...
				If ValType(FORNECEDOR->(FieldGet(nX))) == "C"
					IF FORNECEDOR->(FieldName(nX)) $ 'NOME'
						cJson += '"' + FORNECEDOR->(FieldName(nX)) + '": "' +   EncodeUTF8( FWNoAccent(cValToChar(AllTrim(FORNECEDOR->(FieldGet(nX))))) )  + '"' + CRLF
					Elseif FORNECEDOR->(FieldName(nX)) $ 'NOME_FANTASIA'
						cJson += '"' + FORNECEDOR->(FieldName(nX)) + '": "' +   ( FWNoAccent(cValToChar(AllTrim(FORNECEDOR->(FieldGet(nX))))) ) + '"' + CRLF
					Else 
						cJson += '"' + FORNECEDOR->(FieldName(nX)) + '": "' +   ( FWNoAccent(cValToChar(AllTrim(FORNECEDOR->(FieldGet(nX))))) ) + '"' + CRLF
					EndIf
				Else
		...
		For nX := 1 to nTot
			...
			If SA2->(dbSeek(xFilial("SA2") + CCNPJ ) )
				...
				aFornece := {{"A2_FILIAL"   , SA2->A2_FILIAL														    															 ,Nil},; //filial     -C-06
							...
							{"A2_XSITE" 		, iif(ValType(aDados[nX]["A2_XSITE"])  <> "U", aDados[nX]["A2_XSITE"], "")   						 ,Nil},; //Link do site
							{"A2_TIPOST" , iif(ValType(aDados[nX]["A2_TIPOST"])  <> "U", aDados[nX]["A2_TIPOST"], "")   			 ,Nil}} 	//Tipo do site, interno ou externo					 					 						 						 							
