				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							...
							EndIf
								//?????????????????????????????????????????????????????????????????????????J??				
								//     RAFAEL ALMEIDA - SIGACORP (IDENTIFICA??O DE CUSTOMIZA??O)  - INICIO 
								//     21/03/2014 
								// Macedo - Cnsultor (MS-01)
								//?????????????????????????????????????????????????????????????????????????J??

								IF !AllTrim(SC5->C5_MENSA1) $ cMensCli //29/08/16 - Fabio Yoshioka Cria??o de verifica??o para evitar problemas em repetir mensagens
									cMensCli += AllTrim(SC5->C5_MENSA1) + CRLF         // MACEDO : TRATAMENTO DAS MENSAGENS NO PED DE VENDAS
								ENDIF

								IF !AllTrim(SC5->C5_MENSA2) $ cMensCli							
									cMensCli += AllTrim(SC5->C5_MENSA2) + CRLF           // DEVE SER LEVADA PARA NOVAS VERSOES DESTE PROGRAMA ...
								ENDIF

								cMensCli += AllTrim(SC5->C5_MENNOTA)+ CRLF          // ESPECIFICO ALUBAR...

								IF SM0->M0_CODIGO="06" 
									IF !AllTrim(SC5->C5_MENSA3) $ cMensCli								
										cMensCli += AllTrim(SC5->C5_MENSA3) + CRLF
									ENDIF

									IF !AllTrim(SC5->C5_MENSA4) $ cMensCli																
										cMensCli += AllTrim(SC5->C5_MENSA4) + CRLF
									ENDIF

									IF !AllTrim(SC5->C5_MENSA5) $ cMensCli																	
										cMensCli += AllTrim(SC5->C5_MENSA5)
									ENDIF
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
							...
							@aTotICMSST,len(aProd), nX, @nValDifer, cIndPres, lExpCDL, @aMonof02, @aMonof15, @lMonof53, @lMonof61, aICMSMono[nX] )
	Next nX
