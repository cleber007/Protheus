...
IF LEN(cStr) == 48 .AND. lRet
	...
	WHILE nConta <= 4
		...
		// Retirada validacao digito verificador
		lRet    := IF(lRet,(nDVCalc==nDV),.T.)
		lRet:=.T.
		nConta  := nConta + 1
...
IF LEN(cStr) == 44 .AND. lRet
	...
	IF cTipo == "C" .OR. (cTipo == "I" .AND. !lRet)
		...
		nDVCalc := IF(nDVCalc==10,0,nDVCalc)
		lRet    := IF(lRet,(nDVCalc==nDV),.T.)
		//Retirada validacao codigo de barras
		lRet :=.T.
...
If !Empty(cStr)
	If Substr(cStr,1,1) <> "8" .and. Val(SubStr(cStr,6,4)) > 0
		...
		EndIf
	EndIf
	If Substr(cStr,1,1) <> "8" .and. Val(SubStr(cStr,10,10)) > 0
		...
		EndIf
	EndIf
	If Substr(cStr,1,1) == "8" .and. Val(SubStr(cStr,6,10)) > 0
...
+-----------------------------------------------------------------------------+
|Alterado                                         |Em      | Por              |
|Incluir configuração Tributos                    |15/07/09|                  |                                                                                                                             |
...
_Abat := SE1->E1_VALOR-SomaAbat(SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,"R",SE1->E1_MOEDA,dDataBase,SE1->E1_CLIENTE,SE1->E1_LOJA)
