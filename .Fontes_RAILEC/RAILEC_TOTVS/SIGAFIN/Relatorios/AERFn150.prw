		...
		If lContinua
			...
			Else
				...
				If TCSqlExec(cQryBx) < 0
					FR150Log(STR0068 + TCSQLError() , "QueryBx")//"Ocorreu um problema na execução do comando SQL no seu banco de dados, avalie os detalhes:"
					lMstErro := .T.
				EndIf
			EndIf
			...
			If lMstErro // Alerta o usuário que o
			...
			While &cCond1 .And. SE2->(!EoF()) .And. lContinua .And. SE2->E2_FILIAL == xFilial("SE2")
				...
				While &cCond2 == cCarAnt .And. !EoF() .And. lContinua .And. SE2->E2_FILIAL == xFilial("SE2")
					...
					If MV_PAR21 == 1
						...
						//Verifica se existem compensações em outras filiais para descontar do saldo, a SaldoTit
						//somente verifica as movimentações da filial corrente, Nao deve processar quando existe somente uma filial.
