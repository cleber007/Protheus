...
// -----------+-------------------+---------------------------------------------------------
// 15/02/2021 | Amarilson Ribeiro    | Gerado com auxílio do Assistente de Código do TDS.
// -----------+-------------------+---------------------------------------------------------
...
User Function AJUSTIT(_atit)
	...
	Local aACampos  	:= {"E2_CODBAR","A2_BANCO","A2_AGENCIA","A2_NUMCON","A2_DVCTA","A2_DVAGE"} //Variável contendo o campo editáveis no Grid
...
Static Function CriaCabec()
	...
	aadd(aCabecalho, {"Código Barra", "E2_CODBAR" , "@!"             , 48, 0, "", "", "C", ""   , "R", "", "", ""})
...
User Function NEWBBOR

	cQuery += "SELECT MAX(EA_NUMBOR) "
	cQuery += "WHERE "
	cQuery += "EA_NUMBOR >= '" + cNumBor + "' AND D_E_L_E_T = ' '"
	...
	While QBOR->(Eof())
		MsgAlert("Numero do borderô" + cNumBor + "já foi utilizado, por gentileza fechar a rotina e abrir novamente.")
	Enddo
