...
User Function Exec()
	...
	oDlg1      := MSDialog():New( 092,232,592,927,"Exec - Executando Programas ADVPL",,,.F.,,,,,,.T.,,,.T. )
	...
	oSay5      := TSay():New( 167,011,{||'fNome("Parametro")'},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,073,008)
	oSay6      := TSay():New( 001,081,{||"Bem vindo "},oDlg1,,oFont1,.F.,.F.,.F.,.T.,CLR_BLUE,CLR_WHITE,187,016)
...
/*
	if !FindFunction("cNomeFun")
		MsgInfo("Função não localizada", "Atenção")
		Return
	Endif
*/
return &cNomeFun
